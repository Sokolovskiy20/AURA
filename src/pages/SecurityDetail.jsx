...
const SecurityDetail = () => {
  ...
  return (
    <div style={{ paddingTop: '100px', backgroundColor: 'var(--bg-primary)', minHeight: '100vh', overflow: 'hidden' }}>
      ...
      <section style={{ backgroundColor: 'var(--bg-secondary)', padding: '60px 0 0 0', position: 'relative', overflow: 'hidden' }}>
        ...
        <div className="container" style={{ textAlign: 'center', position: 'relative', zIndex: 1 }}>
          <motion.div initial="hidden" animate="visible" variants={fadeUp} style={{ maxWidth: '800px', margin: '0 auto 60px' }}>
            <h1 className="title-hero" style={{ marginBottom: '20px' }}>Безопасность</h1>
            <p className="subtitle">Ваш дом — ваша неприступная цифровая крепость.</p>
          ...
          >
            <img src="./images/prod_security.jpg" alt="Безопасность" style={{ width: '100%', display: 'block' }} />
          </motion.div>
        </div>
      </section>
      ...
      <section className="section" style={{ backgroundColor: 'var(--bg-primary)', paddingTop: '120px' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <div className="flex-col-mobile" style={{ display: 'flex', flexDirection: 'row', gap: '20px', height: '400px', width: '100%' }}>
            {[
              { title: "Face ID для дома", desc: "Биометрические замки узнают владельцев в лицо и автоматически открывают дверь без ключей." },
              { title: "Локальный сервер", desc: "Никаких облаков. Все данные хранятся локально. Взломать систему снаружи практически невозможно." },
              { title: "Защита от протечек", desc: "Датчики фиксируют воду за секунду, а электроприводы мгновенно перекрывают стояки, спасая ремонт." },
              { title: "Имитация присутствия", desc: "Когда вы в отпуске, дом сам включает свет и шторы, создавая иллюзию присутствия." }
            ].map((tech, idx) => {
      ...
      {/* 3. СЦЕНАРИИ (ДОМА / В ОТЪЕЗДЕ) */}
      <section style={{ backgroundColor: '#000', minHeight: '100vh', overflowX: 'hidden' }}>
      <Helmet>
        <title>Система Безопасности | Продукты AURA</title>
        <meta name="description" content="Многоуровневая система безопасности AURA. Невидимая защита." />
      </Helmet>
        ...
        <div className="container" style={{ position: 'relative', zIndex: 1, textAlign: 'center' }}>
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "0px" }} variants={fadeUp} style={{ marginBottom: '60px' }}>
            <h2 className="title-section" style={{  }}>Сценарии охраны</h2>
            <p className="subtitle" style={{ maxWidth: '800px', margin: '16px auto 0' }}>Нажмите, чтобы изменить режим безопасности</p>
            ...
            <div className="flex-col-mobile" style={{ display: 'flex', justifyContent: 'center', gap: '16px', marginTop: '40px' }}>
              <button 
                ...
                style={{ 
                  ...
                  background: !isAway ? 'rgba(229,195,135,0.1)' : 'transparent',
                  color: !isAway ? 'var(--accent-gold)' : 'var(--text-secondary)',
                  cursor: 'pointer',
          ...
          <div style={{ borderRadius: 'var(--radius-lg)', overflow: 'hidden', height: '60vh', position: 'relative', boxShadow: '0 30px 60px rgba(0,0,0,0.5)' }}>
            <img 
              src="./images/apple_security.jpg" 
              alt="Home Scene" 
              style={{ 
                position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover', 
                opacity: isAway ? 0 : 1, transition: 'opacity 1s ease-in-out' 
              }} 
            />
            {/* Имитация спокойного режима дома */}
            <div 
              style={{ 
                position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', 
                backgroundColor: '#000', mixBlendMode: 'overlay',
                opacity: isAway ? 0 : 0.3, transition: 'opacity 1s ease-in-out', zIndex: 1
              }}
            ></div>

            {/* Имитация режима охраны */}
            <img 
              src="./images/apple_security.jpg" 
              alt="Away Scene" 
              style={{ 
                position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', objectFit: 'cover',
                opacity: isAway ? 1 : 0, transition: 'opacity 1s ease-in-out',
            ...
            <div 
              style={{ 
                position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', 
                backgroundColor: '#100505', mixBlendMode: 'multiply',
                opacity: isAway ? 0.8 : 0, transition: 'opacity 1s ease-in-out', zIndex: 1
              }}
            ></div>
            {/* Лазерная сетка датчиков */}
            <div 
              style={{ 
                position: 'absolute', top: '10%', left: '50%', transform: 'translateX(-50%)', width: '80%', height: '50%',
                background: 'radial-gradient(ellipse, rgba(255,0,0,0.3) 0%, rgba(0,0,0,0) 70%)',
                opacity: isAway ? 1 : 0, transition: 'opacity 1s ease-in-out', zIndex: 2, pointerEvents: 'none', mixBlendMode: 'screen'
              }}
            ></div>
            ...
            <div style={{ position: 'absolute', bottom: '40px', left: '40px', zIndex: 3, textAlign: 'left', textShadow: '0 2px 10px rgba(0,0,0,0.8)' }}>
              <h3 style={{ fontSize: '2rem', color: '#fff', marginBottom: '8px' }}>
                {isAway ? 'Полная постановка на охрану' : 'Периметр под защитой'}
              </h3>
              <p style={{ color: 'rgba(255,255,255,0.8)', fontSize: '1.1rem' }}>
                {isAway ? 'Активированы внутренние камеры, датчики движения и имитация присутствия.' : 'Камеры внутри отключены для приватности. Внешний контур на страже.'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="container">
          <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, margin: "0px" }} variants={fadeUp} style={{ textAlign: 'center', marginBottom: '80px' }}>
            ...
            <p className="subtitle" style={{ maxWidth: '800px', margin: '16px auto 0', textTransform: 'uppercase', letterSpacing: '0.05em', fontSize: '0.9rem' }}>
              От аудита до криптографической защиты
            </p>
          </motion.div>
          ...
          <div style={{ position: 'relative', maxWidth: '800px', margin: '0 auto' }}>
            ...
            <div className="flex-col-mobile" style={{ display: 'flex', flexDirection: 'column', gap: '60px' }}>
              {[
                { step: "01", title: "Аудит", desc: "Анализируем уязвимости объекта и проектируем многоуровневый контур защиты." },
                { step: "02", title: "Проектирование", desc: "Разрабатываем схемы прокладки слаботочных сетей и резервного питания серверов." },
                { step: "03", title: "Монтаж", desc: "Скрытно устанавливаем камеры, датчики периметра и биометрические замки." },
                { step: "04", title: "Шифрование", desc: "Настраиваем локальные серверы и криптографическую защиту каналов связи." }
              ].map((item, idx) => (
      ...
      <section className="section" style={{ backgroundColor: 'var(--bg-secondary)' }}>
        <div className="container" style={{ textAlign: 'center' }}>
          ...
          <div className="reviews-grid">
            {[
              { text: "Я могу спать спокойно, зная, что мой дом находится под круглосуточной надежной защитой серверов AURA.", author: "Александр, Пентхаус" },
              { text: "Face ID замки — это нечто! Больше никаких ключей, дверь просто открывается, когда я подхожу.", author: "Евгений, Резиденция" },
              { text: "Датчики протечки уже один раз спасли мой ремонт. Инвестиция, которая окупилась моментально.", author: "Михаил, Апартаменты" }
            ].map((review, idx) => (
              <motion.div 
                key={idx}
                initial="hidden" whileInView="visible" viewport={{ once: true, margin: "0px" }} 
